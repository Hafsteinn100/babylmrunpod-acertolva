...
Downloads data from HuggingFace as ZIP files, extracts, and trains.
Works reliably on RunPod with limited disk space.
"""
import argparse
import os
import json
...
EMBED_DIM = 64
HIDDEN_DIM = 400  # Reduced to safely fit under 1MB
VOCAB_SIZE = 256
SEQ_LEN = 128
BATCH_SIZE = 64
...
def download_data():
    """Download IGC data from HuggingFace and extract."""
    from huggingface_hub import snapshot_download
    
    if os.path.exists("IGC-data"):
        print("Data already exists, skipping download...")
        return "IGC-data"
    
    print("Downloading IGC data from HuggingFace...")
    snapshot_download(
        repo_id="arnastofnun/IGC-2024",
        repo_type="dataset",
        local_dir="IGC-snapshot",
    ...
    for root, dirs, files in os.walk("IGC-snapshot"):
        for f in files:
            if f.endswith(".zip"):
    ...
    import shutil
    shutil.rmtree("IGC-snapshot", ignore_errors=True)
